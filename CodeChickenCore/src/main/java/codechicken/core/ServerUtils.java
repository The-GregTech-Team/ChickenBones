...
public class ServerUtils extends CommonUtils
{
    ...
    public static GameProfile getGameProfile(String username) {
        ...
        try {//use reflection to bypass saving the game profiles every time we ask the cache for one
            Object cacheEntry = ((Map) field_152661_c.get(mc().getPlayerProfileCache())).get(username);
            if(cacheEntry != null)
    ...
    public static boolean isPlayerOP(String username) {
        ...
        return prof != null && Arrays.asList(mc().getConfigurationManager().getOppedPlayerNames()).contains(username);
    }
